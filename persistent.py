...
from settings import D_GUILD_ID, D_REPORT_CHAN_ID, DISCORD_API_SECRET, TESTCOGS_DIR
import settings
import pathlib
import random


# from cogs.reportcog import ReportModal
...
class PersistentView(discord.ui.View):
    ...
    async def button1callback(
        ...
        await interaction.response.send_message(
            "Your report has been submitted. Salute!", ephemeral=True
        )
...
class PrometheusBot(commands.Bot):
    ...
    async def on_ready(self):
        print(
            f"{Fore.GREEN}{Style.BRIGHT}{self.user}\
            {self.user.id}{Style.RESET_ALL}"
        )
        print("---------")
...
async def prepare(ctx: commands.Context, channel: discord.TextChannel = None):
    ...
    await channel.send(
        "To report, pick the corresponding items\
    and add a discription. You can check your report before submitting",
        view=PersistentView(),
    )
